...
export default function Onboarding({ onDone }: { onDone: () => void }) {
  ...
  // Avvisi sulle spezie restituiti dal server: si mostrano una volta, prima della coda
  ...
  // Spezie scritte fra i cibi non graditi: prima della coda commerciale c'è una cosa da dirle.
  // Non è una formalità — se non gliela diciamo, lei crede di aver escluso il curry e invece no.
  if (result && avvisiSpezie && avvisiSpezie.length > 0) {
    return (
      <div className="app-frame">
        <div className="screen no-tabbar center" style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', gap: 14, padding: 24 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 9 }}>
            <i className="ti ti-alert-circle" style={{ color: '#C98A2E', fontSize: 22 }} />
            <h2 style={{ margin: 0, fontSize: 18 }}>Una cosa sulle spezie</h2>
          </div>
          {avvisiSpezie.map((a) => (
            <div key={a.termine} style={{ padding: '12px 13px', borderRadius: 12, background: '#FDF6E8', border: '1px solid #F0DFBA' }}>
              <b style={{ fontSize: 13.5, color: '#6B4E12' }}>{a.titolo}</b>
              <div style={{ fontSize: 12.5, lineHeight: 1.55, color: '#5C4A22', marginTop: 5 }}>{a.testo}</div>
            </div>
          ))}
  ...
  /**
   * ⚠️ Ha rifatto il questionario togliendo un'allergia (o saltando quella pagina), e noi l'abbiamo
   * TENUTA. Va detto qui, prima di andare avanti.
   *
   * Tenerla in silenzio sarebbe metà lavoro: lei crede di averla tolta, i menu continuano a
   * escluderla, e la prossima volta che ne parla con la coach nessuna delle due capisce perché
   * quel piatto non arriva mai. Il perché lo spiega la frase: dal questionario si aggiunge, si
   * toglie parlando con la nutrizionista.
   */
  if (result && avvisiEsclusioni && avvisiEsclusioni.length > 0) {
    return (
      <div className="app-frame">
        <div className="screen no-tabbar center" style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', gap: 14, padding: 24 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 9 }}>
            <i className="ti ti-alert-triangle" style={{ color: '#C98A2E', fontSize: 22 }} />
            <h2 style={{ margin: 0, fontSize: 18 }}>Allergie e intolleranze</h2>
          </div>
          {avvisiEsclusioni.map((t) => (
            <div key={t} style={{ padding: '12px 13px', borderRadius: 12, background: '#FDF6E8', border: '1px solid #F0DFBA' }}>
              <div style={{ fontSize: 12.5, lineHeight: 1.55, color: '#5C4A22' }}>{t}</div>
            </div>
          ))}
          <button className="btn" style={{ width: '100%', justifyContent: 'center', padding: 12 }} onClick={() => setAvvisiEsclusioni(null)}>
            Ho capito, andiamo avanti
          </button>
        </div>
      </div>
