...
- `[Prodotto]` **App cliente/Onboarding — campi anagrafici + schermata privacy** — nel prototipo (e `docs/`): pag.19 "Come vuoi essere chiamata?" ora ha **Età + Sesso (Uomo/Donna)** (tolta l'altezza); pag.20 "Il tuo punto di partenza" ha in più il campo **Altezza (cm)**; **nuova schermata "Trattamento dei dati personali"** (GDPR, dati sanitari solo al nutrizionista, consenso) inserita **dopo pag.24 (colore)**. Flusso ora **35 passi** (conteggio dinamico). Verificato a runtime con jsdom. → **voce da rigenerare** (solo 2 chiavi): `FORCE=1 ONLY="q17_resto,privacy"` (q17_resto testo cambiato, privacy nuova). → impatto [Sviluppo]: replicare i campi e la schermata consenso nell'app React.
