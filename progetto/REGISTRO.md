...
- `[Sviluppo]` **Liste CRM — Fase A (raggruppamenti manuali + campi storici)** — nuove entità `CrmList` e `CrmListMember` (N:N: un contatto in più liste), + `previous_status` e `historical_paid_cents` sul lead (storico pre-Metabole, solo informativo, NON entra in contabilità). Migrazione `20260714240000`. Backend: `/crm/lists` CRUD (coach/sales/capo/admin), `POST /crm/leads/:id/lists` (rimpiazza le appartenenze), filtro `?listId=` e campo `lists` in list/detail, `updateInfo` estesa ai campi storici; audit su tutto. Backoffice: **LeadDetail** con badge liste, editor a chip (aggiungi/togli + "crea e aggiungi"), campi storico; **LeadsTable** con filtro per lista, badge per riga e modale "Gestisci liste" (crea/rinomina/elimina). Test: +4 (listLists, setLeadLists dedup/NotFound, detail appiattito) → suite finance-crm 15/15 verde. Type-check backoffice 0 errori. **Resta la Fase B: import CSV/XLSX delle liste storiche.**
