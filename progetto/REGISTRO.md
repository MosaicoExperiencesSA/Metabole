...
- `[Sviluppo]` **Provvigioni/compensi PER PRODOTTO in € (via dai parametri globali)** — le 4 percentuali in `config_param` (coach, manager coach, nutrizionista, capo nutrizionista) sono state SOSTITUITE da 4 importi FISSI in centesimi su ogni **piano** e **prodotto** del negozio (`commission*Cents`). `generateCommissions` ora somma le quote dal piano dell'abbonamento e/o dai prodotti dell'ordine (× qty) e le riscala sull'importo pagato (sconto → riduzione proporzionale, come richiesto). Rimosso il flag `commissionTeam` (0 su un lato = niente provvigione). Migrazione `20260714230000`: +8 colonne, drop `product.commission_team`, DELETE dei 4 config_param. Backoffice: Gestione negozio con 4 campi € (piani e prodotti) + riepilogo in tabella; Parametri: rimossa la sezione %. ⚠️ **Dopo il deploy i piani/prodotti esistenti hanno provvigioni a 0 finché l'admin non le imposta.** Test: finance-crm aggiornato (importi €) + fix DI onboarding spec (mock PersonalBaseService). Suite backend verde in sandbox (transpile-only, stub Prisma), backoffice type-check 0 errori.
