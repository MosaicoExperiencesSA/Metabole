...
- `[Sviluppo]` **Storno acquisti (rimborso registrato)** — dalla tabella Acquisti (admin): `POST /admin/purchases/:id/refund` con importo deciso dall'operatore (anche parziale) + nota. Effetti in transazione: campi `refund_*` sul pagamento (migrazione `20260714220000`), **blocco menu** (abbonamento → cancelled), incasso negativo a ledger (stessa categoria: report nettati da soli), **storno proporzionale delle provvigioni** (ledger negativo, compenso del periodo ridotto con voce `sales_commission_refund`, accantonate ridotte), audit. Alla cliente email it/en con **ricevuta di rimborso PDF** (`RMB-…`, anche scaricabile dal backoffice). UI: pulsante Storno + modal con avvertenze, chip "Stornato −€", filtro stato "Stornato". Il rimborso effettivo su Stripe/bonifico resta manuale (deciso con Simone). Type-check backoffice 0 errori.

- `[Sviluppo]` **Team assegnato dal responsabile (non più in automatico) · ref code anche nutrizionista · tabella Acquisti · schermate onboarding** — ① l'onboarding NON assegna più coach/nutrizionista in automatico (rimossa `pickLeastLoadedStaff`): li assegna il responsabile dal backoffice; unica eccezione il ref code in registrazione, ora valido anche per una **nutrizionista** (colonna `crm_record.assigned_nutritionist_id`, migrazione `20260714210000`). ② Acquisti: filtri per ogni colonna + più recenti in alto (fix `listPayments` asc→desc). ③ App: via la "Prima visita" (percorso pronto + nel frattempo); testi per team non ancora assegnato. Test aggiornati; type-check app+backoffice 0 errori.
