...
- `[Sviluppo]` 🕐 **L'orologio del digiuno diventa una porta: `GET`/`PATCH /me/digiuno`, il passo
  ...
  quello che ha chiesto e quello che è in vigore sono diversi apposta. ⛔ **La prima scelta non è un
  cambio** e ha un ramo suo: nessun limite di uno al giorno addosso a chi risponde per la prima
  volta a una domanda che non le era mai stata fatta. ⛔ **La revisione ha trovato otto cose, due
  gravi.** *«Vale da domani» non lo faceva rispettare nessuna riga*: alle 14:00, dopo che aveva già
  pranzato, il sistema le diceva «da domani apri alle 16:00» e intanto scriveva le 16:00 — l'orologio
  in home le disegnava la giornata di oggi col primo pasto alle 16:15. Adesso quello che vale da
  domani va nei **campi bersaglio** e lo applica lo stesso cron del piano graduale: un meccanismo
  solo per due cose che sono la stessa cosa, «questo vale dalla prossima apertura». E *un tocco a
  vuoto la bloccava per venti ore*: un `PATCH` che non cambia niente — doppio tocco, retry dell'app —
  scriveva comunque «spostata adesso», e lo spostamento vero dieci minuti dopo si prendeva «puoi
  rifarlo fra 20 ore». ⚠️ Altre tre: la frase «stanotte il digiuno dura X» era falsa quando cambiava
  anche il protocollo; alla nutrizionista si raccontava una pagina mai aperta; e le si dava
  un'istruzione che il sistema disfa («correggi dalla scheda» — ma la finestra la deriva l'orologio).
  ⚠️ Nuova `oraLocaleInMinuti` in `date-only.ts`: su Render `TZ` non è impostata, e due ore di scarto
  decidono se la finestra si è già aperta. ⚠️ E i test dipendevano dall'ora del giorno — passavano la
  mattina e fallivano dopo pranzo: adesso l'orologio si ferma, con i due casi espliciti.
  ✅ **4374 test su 4374**, ventidue mutazioni su ventidue mordono, build verde.

- `[Prodotto]` 🧪 **Allergia ai solfiti: messa in coda con la guida della nutrizionista.** Il tag
  esiste già fra i 14 UE, ma ⛔ **il dizionario che lo suggerisce ha quattro parole** (`solfiti`,
  `solfito`, `vino`, `aceto di vino`) contro una dozzina di portatori veri: frutta secca
  disidratata, pomodori secchi, crostacei, insaccati, salse pronte, dadi, aceto balsamico, birra.
  E manca il pezzo che la nutrizionista ha mandato davvero, che non è un elenco di divieti ma un
  **prontuario di sostituzioni** — la stessa forma di `menu/lattosio.ts`. ⚠️ Due delle sei
  sostituzioni cambiano il piatto e non l'ingrediente (crostacei → pesce di lisca, insaccati →
  macinato fresco): le decide Lucia prima che qualcuno le scriva.
