...
- **«Sostituisci un ingrediente» → chat con Gaia** ✅ (8/8). Il pulsante della home non apre più
  un pop-up: porta in `/assistente?who=ai&intent=sostituzione` e Gaia comincia lei il dialogo
  (`POST /me/threads/sostituzione`). Chiede **perché**, non «per quanto»: la durata la deduce il
  codice dal motivo, e la sostituzione concordata **viene scritta sulla giornata coi grammi**,
  marcata «da verificare» per la nutrizionista. La ricetta di catalogo non si tocca mai.
  ⚠️ `POST /me/menu/substitute` resta in piedi: lo usano i **cibi esclusi** del Profilo e le app
  già installate (OTA spenti). Progetto e punti aperti: `progetto/PROGETTO_gaia-cambio-menu.md`.
...
- **Conversazioni nella scheda cliente, chat con Gaia compresa** ✅ (8/8) — card `ConversazioniCard`
  in `ClientDetail.tsx` su `GET /staff/clients/:id/threads`, più l'elenco dei **cambi di menu
  concordati in chat** con lo stato di verifica (`/sostituzioni-chat`). Prima il thread `ai` non
  era visibile a nessuno: `staff/threads` vincola il `counterpart` a coach/nutritionist, quindi
  quello che la cliente diceva a Gaia non lo sapeva nessuno.
  ⚠️ Il thread di Gaia si **legge e non si scrive** (`assertThreadAccess` ha ora `read`/`write`):
  in quel thread la voce è di Gaia, e una risposta dello staff travestita da assistente
  ingannerebbe la cliente. Gate: `chat` per le conversazioni, `clients` per l'elenco dei cambi.
