...
- **Sessione 14/07 sera (Claude) — FATTI:** regola ripetizione bigiornaliera (ProductRule `menu_repeat_two_days`); liste CRM + import storico + file arricchito con **codice fiscale/indirizzo** (`Metabole_Import_Pronto_v2.csv`, fuori repo) + campi su CrmRecord (mig `20260714260000`); provvigioni/compensi per prodotto in €; storno acquisti; team assegnato dal responsabile + ref code; **R12 mantenimento efficacia 0,1**; **routing chat sensibile** (medico→nutrizionista, resto→coach primo filtro); pagina **Regole motore** (`/regole-motore`, capo nutrizionista) con regole globali + **14 preset suggeriti per nutrizione** + proposte + applica-a-dieta (mig `20260714270000`); motore legge **override per dieta**; **interfaccia mail** leggibile; **audit dashboard** (28 sezioni con nav+permesso+modulo); permesso `engine_rules` abilitabile al nutrizionista; **PDF istruzioni nutrizionista**; **pagamento reale testato** (gate chiuso); **pulizia dati test/demo eseguita su Render** (`cleanup-demo.ts`). ⚠️ Dopo la pulizia il **catalogo diete/ricette è vuoto** (rimosso anche il catalogo Keto): va ricreato o riseminato prima dello smoke test (serve un menu erogabile). I **gruppi di equivalenza sono rimasti** (non toccati).
...
4. **Igiene pre-apertura**: ~~cancellare il lead di prova "Test GoLive Claude"~~ (fatto con la **pulizia dati test/demo**); verificare segreti Render (`ADMIN_*`, `CORS_ORIGINS`, eventuale `AI_API_KEY`); **IBAN reale** in `bank_transfer_details` (oggi placeholder); conferma prezzi piani/prodotti; **riallineare `progetto/STATO_LANCIO.md`** (segna ancora "base contatori ⬜" e "E2–E5 da fare": sono fatti).

## 2. 🆕 Nuove richieste (Simone, 14/07 sera)

### 2.1 Regola motore menu — ripetizione bigiornaliera decisa dal nutrizionista

**✅ FATTO (14/07)** — implementata come `ProductRule` `menu_repeat_two_days` **per prodotto/dieta** (decisione socio, non per cliente), OFF di default. Giorno 2+ ripropone gli stessi alimenti del giorno 1 con ricetta diversa (la "gemella"): stesso **gruppo di equivalenza approvato** + kcal in banda (`repeat_twin_kcal_tolerance_pct`, def. 15) + miglior punteggio efficacia/gradimento. Fallback socio: se manca la gemella resta il **pasto già composto** (nuovo), non la ripetizione identica. Config_param `menu_repeat_two_days_default` + `repeat_twin_kcal_tolerance_pct` a seed. **Inerte** finché il nutrizionista non approva i gruppi di equivalenza. Test +3 verdi. ⏳ Resta il **toggle nella scheda dieta del backoffice** (accensione via UI; oggi la regola si attiva creando la `ProductRule` — parte del wiring "regole per prodotto" §motore). ✅ **AGGIORNAMENTO 14/07 sera:** costruita la pagina **Regole motore** (`/regole-motore`, solo capo nutrizionista): regole globali editabili, **14 regole suggerite per tipo di nutrizione** (5 stili + DASH/Vegetariana/Vegana/Pescetariana/Flexitariana/Iperproteica/Basso IG/Digiuno/Mediterranea ipocalorica) col flag *suggerita* — modificabili, aggiungibili e **applicabili a una dieta** (→ ProductRule) — e **proposte** di regole nuove. Modulo dashboard. Così l'accensione delle ProductRule ha una UI (incluso `menu_repeat_two_days`). Resta follow-up: far leggere al motore gli **override numerici per dieta** (oggi legge i globali + gli interruttori bigiornaliera/DayCombo).
