...
export function controllaRicettaDaScrivere(r: RicettaDaScrivere): EsitoScrittura {
  ...
  if (esito.tipo === 'sicura' && quantoLargo(regime) < quantoLargo(esito.regimeGiusto)) {
    return {
      esito: 'conferma',
      problema:
        `Fra gli ingredienti c'è «${esito.prova}»: un piatto dichiarato ${regime} non dovrebbe contenere ${esito.cosa}. `
        + 'Se è una versione vegetale, scrivilo nel nome dell\'ingrediente («formaggio vegano», «ricotta di mandorla») e non chiederà più; '
        + `altrimenti correggi l'elenco, oppure dichiaralo ${esito.regimeGiusto}.`,
    };
  }

  /**
   * ⛔ **LE UOVA E I LATTICINI IN UN PIATTO VEGANO** — chiusi il 4/9 sera, e dal 5/9 li giudica
   * `classifica` (in `etichetta-contro-contenuto.ts`), la stessa testa dello script che rietichetta
   * il catalogo e delle attività coach: qui non c'è più un secondo giudizio da tenere allineato.
   * Il perché (i derivati vegetali, «formaggio vegano» che resta latte per gli allergeni ma qui non
   * chiede) sta scritto là, accanto a `uovaOLatticini`.
   */
  return { esito: 'ok' };
}
