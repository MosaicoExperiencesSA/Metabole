...
export const ENGINE_RULES: EngineRule[] = [
  ...
  { code: 'menu_variety_famiglia_gap_days', label: 'Distanza minima stesso ingrediente principale (giorni)', description: 'Giorni minimi prima che nello stesso pasto torni un piatto con lo stesso ingrediente principale: due frittate di fila sono due piatti diversi ma la stessa cosa nel piatto. 1 = non due giorni di fila, 0 = spenta. Se il pool non offre alternative il piatto resta.', category: 'varieta', kind: 'number', default: 1, min: 0, max: 7, step: 1, unit: 'giorni', perDiet: true },
  ...
  // ⛔ Il salto impossibile (28/8): non è il calo rapido. Quello misura un corpo che cala troppo in
  // fretta; questo misura un numero che non può essere vero. Le due caselle valgono INSIEME.
  { code: 'weight_jump_impossible_kg', label: 'Pesate incoerenti: salto (kg)', description: 'Di quanti chili devono differire due pesate consecutive perché non ci si possa più fidare del dato. Vale insieme al ritmo qui sotto: servono tutt’e due (dieci chili in dieci giorni). ⚠️ Sotto queste soglie c’è la fisiologia vera — prima settimana di piano su una persona molto pesante, ritenzione, diuretici, rientro dalle vacanze: abbassarle vuol dire suonare su clienti normali, cioè spegnere l’avviso. Quando scatta, quella cliente mangia il livello della sua dieta invece del suo fabbisogno, e l’avviso arriva alla coach e al nutrizionista. ⚠️ Il minimo è 5 e non 2 apposta: con un valore troppo basso quasi ogni coppia di pesate risulterebbe incoerente e il fabbisogno si spegnerebbe per quasi tutte le clienti, in silenzio.', category: 'sicurezza', kind: 'number', default: 10, min: 5, max: 40, step: 0.5, unit: 'kg' },
  { code: 'weight_jump_impossible_kg_week', label: 'Pesate incoerenti: ritmo (kg/settimana)', description: 'Il ritmo implicito fra le due pesate oltre il quale, insieme al salto in chili, il dato non è più affidabile. Senza questa condizione dieci chili in due mesi — un percorso riuscito — suonerebbero come errore. Quando scattano tutte e due le condizioni l’allarme «calo rapido» tace e parla questo: superare da sola questa casella non zittisce niente. ⚠️ Il minimo è 3 e non 1 apposta: con un valore troppo basso quasi ogni coppia di pesate risulterebbe incoerente e il fabbisogno si spegnerebbe per quasi tutte le clienti, in silenzio.', category: 'sicurezza', kind: 'number', default: 7, min: 3, max: 40, step: 0.5, unit: 'kg/sett' },
