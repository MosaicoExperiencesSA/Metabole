...
export const VOCI_INIZIALI: Voce[] = [
...

  {
    ...
    dettaglio:
      ...
      + '\n\n⚠️ **Numeri riallineati il 5/9**: `pages.ts` dichiara **67** chiavi, quelle senza nessun `@RequirePage` sono **33**, e le caselle che **non chiudono la porta** sono **29** — 20 buchi più 9 guardie agganciate che non possono decidere (`accounting` è tutte e due le cose e si conta una volta).'
      + '\n\n✅ **PRIMO PASSO DELLA STRADA (b), 5/9 sera: dieci guardie agganciate** — `audit_logs`, `users`, `roles`, `engine_config`, `pdf_templates`, `shop`, `email_templates`, `email_log`, `accounting`, `accounting_costs`. Tutte su rotte `@Roles(admin)`: nessuno perde l\'accesso, e il giorno che quel `@Roles` si allarga la matrice comincia a comandare da sola.\n'
      + '⛔ **Ma NON hanno chiuso niente, e la revisione l\'ha trovato prima della consegna.** `page.guard.ts` fa uscire l\'admin **prima** di leggere la matrice: dove l\'unico che entra è l\'admin, la guardia non ha nessun esito possibile diverso da «passa». Spegnere quelle caselle toglie ancora la voce di menu e lascia la porta aperta. ⚠️ E il conto stava per scendere da 29 a 19 senza che fosse cambiato niente: il gesto che questa voce vieta per iscritto, preso per una strada laterale che nessuna prova sorvegliava. Adesso c\'è `GUARDIA_INERTE` in `pages.ts`, la pagina Permessi le segnala come i buchi, e il numero le comprende.\n'
      + '▶️ **Come si chiude davvero una di queste**: si allarga `@Roles` ai ruoli che nei default hanno già la chiave, e da lì decide la matrice. Non è più gratis — cambia chi entra — e si fa una chiave per volta, con Simone.\n'
      + '⛔ **E un buco vero trovato dalla revisione**: `admin/payments` è `@Roles(admin, sales)` senza nessuna guardia, e `sales` ha `accounting` nei default. Spegnergli quella casella gli toglie la voce di menu e gli lascia l\'elenco dei pagamenti, le contabili bancarie e i pulsanti di approvazione. ⚠️ Non si chiude agganciando e basta: `sales` ha solo `view`, e le tre scritture chiederebbero `manage` — gli si toglierebbe qualcosa che oggi fa. **È una decisione di Simone.**\n'
      + '▶️ **Da decidere insieme**: la pagina `/contabilita` sta sotto `accounting_costs` ma legge il conto economico, che è `accounting`. Oggi non si vede perché sono tutte e due solo-admin; il giorno che i due permessi smettono di coincidere, chi ha i costi e non il conto economico vede la pagina e il caricamento fallisce.',
  },
  ...
  },
  {
    ...
    dettaglio:
      ...
      + '\n\n✅ **CHIUSA il 5/9 con la guardia a valle, e il numero l\'ha decisa.** La misura del 4/9 su tutte le giornate che esistono: 1 doppione su 66 giornate a rischio, cioè **1,5%**, sotto la soglia del 5% che questo file dichiara — e sotto quella soglia il piano diceva già cosa fare: *una guardia a valle costa meno di un vincolo dentro il prodotto cartesiano*. `scartaChiRipeteUnPiatto` (in `menu/piatti-doppi-nella-giornata.ts`, accanto al conto che l\'ha misurato) toglie dalle combinazioni quelle che ripetono lo stesso piatto in due pasti, e `day-combo.service.ts` la chiama subito dopo lo scarto delle coppie già viste.\n⚠️ **Il ripiego è la metà che conta**: se togliendo i doppioni non resta **nessuna** combinazione si tengono tutte, e l\'esito lo dice (`piattoRipetuto`). Una cliente con molte esclusioni può avere un pool in cui l\'unica giornata componibile ripete un piatto: meglio un piatto ripetuto che nessun menu.\n▶️ **Resta un solo passo, e non blocca niente**: a fine settembre `npm run diag:piatto-doppio` dirà se sui gemelli si supera il 5%; in quel caso il vincolo va messo **dentro** la composizione. La guardia resta comunque: è l\'ultima riga di difesa e non costa niente.',
  },
  {
  ...
  },
  {
    ...
    dettaglio:
      ...
      + '\n\n✅ **CHIUSA il 5/9: la soglia clinica è arrivata.** Lucia, scheda 7 punto 5: *«allarme su ritmo calo > 1,5 kg/settimana per 2+ settimane consecutive **O salto improvviso > 4 kg**»*. La prima metà c\'era già (`max_weight_change_alert_kg_week`, default 1,5, su quattordici giorni); la seconda mancava, ed è esattamente quella che questa voce chiedeva — *«sopra quanti chili, attraverso una sospensione senza pesate in mezzo, si smette di fidarsi del numero?»*.\n✅ `signals/salto-di-peso.ts` (modulo puro, 9 prove) guarda il calo più grosso fra due pesate **consecutive** su novanta giorni — la finestra larga serve: un salto attraverso una sospensione ha per definizione un buco in mezzo. Venti chili in venticinque giorni adesso suonano. Soglia in Parametri (`weight_jump_alert_kg`, 4 kg).\n✅ **E la seconda decisione, quella su chi lo deve sapere** (punto 6: «entrambe le figure»): era già così e adesso è verificato — `decidiDestinatari` notifica la coach **e** la nutrizionista assegnate; `primary` dice solo chi la prende in carico.\n⚠️ **Il guardrail dei dati sporchi non si tocca** (10 kg **e** 7 kg/settimana insieme): quello decide se fidarsi del numero per calcolare il fabbisogno, questo se avvisare una persona. Confonderli vorrebbe dire azzerare il fabbisogno di chiunque cali quattro chili, cioè di un percorso riuscito — ed è la ragione per cui la prima stesura di agosto era stata buttata.',
  },
  {
  ...
  },
  {
    ...
    ordine: 0,
    blocca: false,
    titolo: 'Digiuno intermittente: pubblicato — i tre numeri, le quattro conferme di Lucia, e le finestre morte tolte dalle tendine',
    dettaglio:
      ...
      + '\n\n✅ **CHIUSA il 5/9: i tre numeri sono arrivati e le quattro conferme di Lucia sono nel codice.**\n▶️ **I tre numeri** (lanciati da Simone in produzione): **9 clienti in digiuno**, tutte con i pasti giusti (8 su «salta la colazione», 1 sulla finestra stretta); **catalogo completo**, 12/12 settimane su tutte le varianti, zero unità di lavoro rimaste; **nessuna giornata sotto il fabbisogno** su 96 erogate.\n✅ **E il numero che aspettavamo dal 22/8**: `skip_lunch`, `skip_dinner`, `skip_breakfast_lunch`, `skip_dinner_breakfast`, `skip_morning_snack`, `skip_all_but_dinner` hanno **zero profili** su TUTTI i percorsi. Le tre che l\'orologio non sa disegnare sono uscite dalle tendine (`selezionabile: false`) — restano leggibili in tabella, non cancellate — e con loro sparisce la segnalazione che partiva a Lucia per una scelta fatta cinque minuti prima. Da oggi la sola finestra scegliibile è «salta la colazione», e le prove lo tengono fermo.\n✅ **Le quattro conferme cliniche** (`progetto/guide/Risposte_Cliniche_Lucia_2026-09-05.pdf`, scheda 7): **(1)** controindicazione emersa a digiuno già in corso → **sospensione immediata**, scritta in `profile.service.sospendiDigiuniControindicati()` dentro il giro notturno, con la cliente riportata alla giornata piena, il motivo scritto in scheda (`fastingSospesoIl`/`fastingSospesoPerche`) e una segnalazione clinica che arriva a coach **e** nutrizionista; **(2)** **BMI minimo 18,5** e le **tre domande di esclusione** (DCA, gravidanza o allattamento, ipoglicemizzanti o diabete T1) — giudizio in `menu/digiuno-si-puo.ts` con 14 prove, colonna `fastingExclusions`, caselle in scheda cliente; **(3)** **nomi dei pasti per fascia oraria** (colazione 07-10, pranzo 12-15, cena 19-22, e fuori fascia si torna a «Primo pasto», che è vero sempre): il pasto delle 08:15 non si chiama più «Pranzo»; **(4)** **quote per pasto: restano .45/.10/.45** — Lucia ha **scartato** il 36·16·48 del manuale, e adesso il numero scartato è scritto accanto a quello in uso in `catalog/struttura-per-digiuno.ts`, con una prova, perché chi rilegge il manuale non lo «corregga».\n▶️ **Cosa resta, ed è piccolo e dichiarato**: le tre domande di esclusione oggi si registrano **dalla scheda cliente** (dove emergono davvero, parlando con la cliente); metterle anche nel **questionario dell\'app** è il passo successivo, e finché non c\'è il campo resta vuoto — che vuol dire «non gliel\'abbiamo chiesto», non «va bene»: chi già digiuna non si sospende su un dato che non abbiamo.\n⛔ **E una cosa che NON è ancora vera, detta perché la revisione l\'ha trovata scritta al contrario**: il giudizio `digiunoSiPuoProporre` (BMI + tre domande) è scritto e provato, ma **non lo chiama ancora nessuno** — il questionario continua a offrire il digiuno a chiunque. La protezione che vale oggi è la **sospensione**, che toglie il digiuno a chi ha una controindicazione dichiarata; il filtro in ingresso arriva col campo nel questionario, ed è il passo successivo. Una nota di consegna che dichiara chiusa una protezione inesistente è peggio della protezione mancante.',
  },
  {
...

  {
    ...
    dettaglio:
      ...
      + '\n\n✅ **STRADA (b) FATTA, 5/9: `npm run diag:schede-stile`.** Legge gli stili **pubblicati e '
      + 'visibili** dal database — cioè quelli che una cliente può davvero ricevere, non i preset — e li '
      + 'confronta con le schede del file dell\'app. Dice tre cose separate: chi non ha **nessuna** scheda '
      + '(in registrazione il «?» non compare), chi ce l\'ha **a metà** (il «?» compare e apre un popup '
      + 'svuotato, che è peggio perché sembra una risposta), e **quante clienti stanno oggi** su uno stile '
      + 'scoperto — che è il numero da cui dipende se serve la strada (c). ⚠️ Il giudizio sta in '
      + '`engine-rules/scheda-stile.ts` con le sue prove, non nello script; il lettore che stava dentro la '
      + 'prova del 3/9 è sceso lì, così la prova e la diagnostica non lo riscrivono a modo loro.'
      + '\n⛔ **Lo zero parla**: senza nessuna dieta pubblicata il tabulato lo dice invece di stampare '
      + '«zero stili scoperti», e se il lettore non ricava più nessuna scheda si ferma — un regex rotto '
      + 'farebbe risultare **ogni** stile scoperto, cioè un tabulato di allarmi falsi.'
      + '\n⚠️ **Tre cose corrette in revisione, prima di consegnare, e sono quelle da cui dipende se il '
      + 'numero è vero.** (1) Uno stile con delle clienti sopra e **nessuna dieta visibile** spariva da ogni '
      + 'riga — e la sua scheda finiva fra quelle «che nessuno usa»: il backoffice assegna guardando solo '
      + '`status: approved`, `clientVisible` chiude l\'app e non la scheda. (2) I profili si contavano '
      + '**tutti**, compresi gli account cancellati (la cancellazione lascia il profilo dov\'è) e chi ha '
      + 'compilato il questionario senza mai comprare: il numero era gonfio. (3) Una dieta pubblicata con il '
      + 'codice stile **vuoto** veniva saltata in silenzio, ed è il difetto del 6/8 esatto.'
      + '\n⚠️ E le soglie che decidono «campo scritto» erano tarate così in basso (60) da non vedere una '
      + '`cosaDiceLaRicerca` scritta a un terzo: misurati i testi veri (il più corto è 187), adesso stanno a 100.'
      + '\n▶️ **Resta da lanciarlo** (`npm run diag:schede-stile`) e, col numero in mano, decidere se serve '
      + 'la strada (c). ⛔ E una cosa che non è codice: «cosa dice la ricerca» e «da tenere presente» sono '
...

  {
    ...
    dettaglio:
      ...
      + '\n\n✅ **CHIUSA IL 5/9, tutte e tre.**\n'
      + '1. **Il secondo giro dell\'agente**, `compilaAllergeniMancanti`: chiede solo gli allergeni (mai i valori, che li ha messi una persona) alle righe che nessuno ha ancora guardato, prima quelle che più ricette nominano. La colonna nuova `allergens_filled_by` dice **chi li ha guardati** — l\'agente o una persona — ed è quella che distingue «cercato e non ne ha» da «non lo sa nessuno», visto che sul database sono tutti e due un elenco vuoto. Finché è vuota l\'agente può chiedere; appena qualcuno risponde, non ci scrive più sopra.\n'
      + '2. **Gli scarti si vedono in pagina**, sotto il termine, in arancione: «l\'agente l\'ha bocciato: la fonte non ha un indirizzo · 4/9». Si leggono dal registro per gli id che sono in elenco, e se il registro non risponde la pagina esce come prima.\n'
      + '3. **Il gesto inverso c\'è**: `npm run ritira:tag-alimento` (sola lettura; `CONFERMA=1` scrive). Non toglie un tag che la ricetta avrebbe comunque dalle parole, né uno che **un altro alimento** della tabella giustifica ancora, né tocca le ricette guardate a mano o spuntate in blocco; e si ferma se la riga in tabella dichiara ancora quegli allergeni, perché altrimenti la propagazione di stanotte li rimette e il gesto è stato inutile.',
    categoria: CODICE,
    fatta: true,
    ordine: 679,
    nata: '2026-09-05T11:00',
