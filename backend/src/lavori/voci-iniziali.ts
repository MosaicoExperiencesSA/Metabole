...
export const VOCI_INIZIALI: Voce[] = [
...

  {
    ...
    dettaglio:
      'Trovata dalla revisione avversariale del 19/8 sera, ⚠️ **non è un difetto nuovo**: è come la ricerca degli alimenti ha sempre funzionato. Per rispondere a «quante calorie ha X?» si cercano i nomi della tabella **dentro** il testo della domanda, come pezzi di testo — e i pezzi di testo si incastrano dove non dovrebbero: «melanzane» contiene «mela», «risotto» contiene «riso», «panettone» contiene «pane». ⛔ Il danno è che Gaia risponde con **le calorie dell\'alimento sbagliato**, e il numero è plausibile: 44 kcal per una melanzana (che è la mela) invece di 18 nessuno lo contesta. ⚠️ **Non l\'ho corretta di mia iniziativa**, e la ragione vera non è quella che avevo scritto qui il 19/8 sera: avevo detto «a parole intere si perdono i plurali», ed **è falso** — «melanzana» non è dentro «melanzane», «mela» non è dentro «mele», «carota» non è dentro «carote»: quei casi oggi non funzionano comunque. ⚠️ *Una ragione falsa fa scegliere per il motivo sbagliato, ed è peggio di una scelta sbagliata.* **La ragione vera è che lo stesso meccanismo che sbaglia è quello che salva**: «pomodorini» trova la riga «pomodori» esattamente come «melanzane» trova «mela», e da fuori non si distinguono. ✅ **Il 19/8 ho fatto la misura, non la correzione**: `npm run diag:ricerca` ripassa i messaggi veri delle clienti attraverso il codice di produzione due volte — com\'è oggi e a parole intere — e mette in fila ogni riga che cambia, scritta come coppia «riga trovata ⊂ parola della domanda», così si legge in un secondo quali sono errori e quali aiuti. ⚠️ Il modo di cercare è un parametro del codice vero (`MODO_DI_OGGI` in `nome-dentro-la-domanda.ts`), non una copia dentro la diagnostica: una misura che si riscrive la regola misura la propria copia. ⚠️ **E non stampa il testo dei messaggi**: escono i nomi degli alimenti e la singola parola che li conteneva, mai la frase. ⛔ **Le strade sono quattro e la scelta è tua**, dopo aver letto i numeri: (a) parole intere e basta — spariscono errori e aiuti insieme; (b) parole intere, e se non trova niente riprova come oggi — tiene gli aiuti, ma dove il pezzo di parola è l\'unica strada l\'errore resta identico; (c) niente; (d) chiudere a mano con un **sinonimo** le righe giuste che escono dalla misura, e poi fare la (a) senza perdere nulla — costa lavoro di persone. ⚠️ **Nota a margine trovata strada facendo**: `ValoriNutrizionaliService.cerca` non la chiama più nessuno in produzione (la strada vera è `cercaTutti`, da `schedaPerRisposta`). Non l\'ho tolta — è coperta da test e descrive la stessa regola — ma vale saperlo prima di ragionarci sopra.',
    categoria: SIMONE,
    ordine: 610,
