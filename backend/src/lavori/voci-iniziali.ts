...
export const VOCI_INIZIALI: Voce[] = [
  ...
  },
  {
    ...
    titolo: 'Il generatore di ricette ora lascia un battito a ogni giro: `npm run diag:catalogo`',
    dettaglio:
      'Domanda di Simone del 18/8: «ieri avevamo montato un generatore di ricette automatico, come facciamo a sapere se sta lavorando? possiamo mettere un monitoraggio?». La risposta onesta era **no, o meglio solo quando è andato bene**: la riga nel registro (`engine_rule.preset.generate_catalog`) la lasciava soltanto la generazione **riuscita**. ⚠️ I tre motivi per cui un giro può finire a mani vuote avevano quindi lo **stesso aspetto — nessuna riga**: catalogo completo (tutto bene), AI fuori uso o credito finito (si riproverà, o mai più), e ⛔ **il cron su Render spento o mai creato** (non gira, e non lo dice nessuno). Il terzo è quello che fa danno, perché un cron che non parte non lascia traccia da nessuna parte. Fatto: **① un battito** — `cron.genera_catalogo`, scritto **sempre**, anche quando il giro non fa niente, col motivo e con l\'errore quando c\'è; in un `try` a parte, perché perdere una generazione per un battito sarebbe il rimedio peggiore del male. Da qui in avanti il silenzio vuol dire una cosa sola: non sta girando. **② `npm run diag:catalogo`** — ultimo giro e com\'è andato, quanti giri e quanti errori nella finestra, quante settimane mancano variante per variante (col conto delle clienti sopra), quante unità di lavoro restano, e quante ricette sono nate — di cui quante aspettano la conferma degli allergeni, perché un catalogo che cresce con una coda che non si smaltisce è un catalogo che cresce e non serve a nessuno. ⚠️ E se di battiti non ne trova **nessuno** lo dice a chiare lettere, con le due cause in ordine di probabilità, invece di stampare zeri: «nessun battito» e «tutto a posto» non devono somigliarsi. ⚠️ Il conto delle settimane **non è riscritto**: `statoVarianti` è stato reso pubblico e la diagnostica chiama quello — due punti che rispondono alla stessa domanda divergono, e il giorno che divergono la diagnostica direbbe che manca poco mentre il generatore continua a lavorare. ⚠️ Avvisa anche quando sono passate più di 36 ore dall\'ultimo giro. 4 test. Nessuna migrazione, nessuna scrittura dalla diagnostica.',
    categoria: MANUTENZIONE,
    ordine: 279,
    fatta: true, // 18/8
  },
